...
const PROVIDERS = {
  ...
  // Built-in LSP tool — main-session Claude (and any agent without cclsp/serena
  // installed) carries this. Tool name is bare `LSP` with an `operation:` arg
  // dispatched to the LSP method, not `mcp__<server>__<tool>` shape. Always
  // available — no detection needed.
  builtin: {
    label:        'built-in LSP',
    prefix:       '',
    matchToken:   'LSP',          // matched by exact tool_name === 'LSP'
    tools: {
      ...
      hover:            'hover',
      diagnostics:      null,      // built-in LSP has no get_diagnostics op
      incoming_calls:   'incomingCalls',
      outgoing_calls:   'outgoingCalls',
      overview:         'documentSymbol',
    },
    // documentSymbol primes the LSP server for that file — equivalent role
    // to cclsp's get_diagnostics warmup.
    warmup: { tool: 'documentSymbol', note: 'primes built-in LSP for this file' },
    // Custom renderer — built-in LSP takes (operation, filePath, line, character)
    // instead of mcp-style positional symbol args.
    renderFileCall: (tool, file) =>
      `LSP(operation: "${tool}", filePath: "${file}", line: 1, character: 1)`,
    renderSymbolCall: (tool, sym) =>
      `LSP(operation: "${tool}", ...) // resolve "${sym}" by line+character`,
  },
};
...
function detectProviders() {
  ...
  // Built-in LSP tool — always available in any Claude Code session that
  // exposes the `LSP` tool. Listed last so MCP-based providers (which give
  // richer cclsp-style diagnostics) take precedence in suggestion ordering.
  active.add('builtin');
...
function buildSuggestion(symbol, intent, indent = '  ') {
  ...
  for (const key of providers) {
    ...
    if (typeof prov.renderSymbolCall === 'function') {
      lines.push(`${indent}${prov.renderSymbolCall(toolName, symbol)}  (${prov.label})`);
    } else {
...
function buildWarmupInstructions(indent = '  ') {
  ...
  for (const key of providers) {
    ...
    if (typeof prov.renderFileCall === 'function') {
      lines.push(`${indent}${prov.renderFileCall(prov.warmup.tool, '<any project file>')}`);
    } else {
...
function buildFileWarmupCall(filePath, indent = '  ') {
  ...
  for (const key of providers) {
    ...
    if (!prov?.warmup) continue;
    if (typeof prov.renderFileCall === 'function') {
      lines.push(`${indent}${prov.renderFileCall(prov.warmup.tool, safeFile)}  (${prov.label})`);
    } else {
...
function isLspProviderTool(toolName) {
  ...
  // Built-in LSP tool: bare `LSP` tool name (no mcp__ prefix)
  if (toolName === 'LSP') return true;
  if (!toolName.startsWith('mcp__')) return false;
  for (const key of Object.keys(PROVIDERS)) {
    const token = PROVIDERS[key].matchToken;
    if (token === 'LSP') continue;  // already handled above
